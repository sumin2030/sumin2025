            ...
            <div class="viewer__inner">
                ...
                <div class="viewer__content">
                    <div class="image-container" id="imageContainer">
                        <img id="mainImage" alt="${imageData.title}" loading="eager" />
                    </div>
...
function showImageViewer(imageId) {
    ...
    if (main) {
        ...
        if (mainImage) {
            ...
            // 이미지 로드 함수
            function loadImageWithFallback(imageSrc, retryCount = 0) {
                const preloadImage = new Image();
                let loadTimeout;
                
                // 절대 경로로 변환 (Netlify 환경 대응)
                const absoluteSrc = imageSrc.startsWith('http') ? imageSrc : 
                    (imageSrc.startsWith('/') ? imageSrc : `/${imageSrc}`);
                
                preloadImage.onload = function() {
                    // preload 성공 시 실제 이미지 로드
                    clearTimeout(loadTimeout);
                    mainImage.src = absoluteSrc;
                ...
                preloadImage.onerror = function() {
                    ...
                    // 재시도 로직 (최대 2번)
                    if (retryCount < 2) {
                        console.log(`이미지 로드 실패, 재시도 중... (${retryCount + 1}/2)`);
                        setTimeout(() => {
                            loadImageWithFallback(imageSrc, retryCount + 1);
                        }, 1000);
                        return;
                    }
                ...
                // 이미지 로드 시작
                preloadImage.src = absoluteSrc;
            }
            
            // 이미지 로드 시작
            loadImageWithFallback(data.src);
